            ...
            <v-table density="compact" class="text-caption" v-if="item.incoming_outgoing_items?.length">
              ...
              <tbody>
                ...
                <tr class="font-weight-bold bg-grey-lighten-4">
                  ...
                  <td>{{ item.incoming_outgoing_items.reduce((sum, i) => sum + (Number(i.weight_society) || 0), 0) }}</td>
  ...
  methods: {
    ...
    fetchRecords() {
      axios
        ...
        .then((response) => {
          this.records = response.data.filter((entry) => entry.is_incoming === false)
        })
        .catch((error) => {
    ...
    downloadReport() {
      ...
      axios
        .get(
          `${config.apiBaseUrl}/api/${config.version}/incoming_outgoing/download_outgoing_report?${params.toString()}`,
          {
            responseType: 'blob',
            headers: {
        ...
        .then((response) => {
          ...
          link.href = url
          link.setAttribute('download', 'outgoing_report_filtered.xlsx')
