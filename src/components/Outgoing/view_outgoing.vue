  ...
  <v-container fluid>
    <v-card elevation="2" class="rounded-lg h-100">
      <v-card-title class="text-h5 font-weight-bold text-center py-4 bg-primary text-white">
        Outgoing Records (जावक रिकॉर्ड)
      </v-card-title>
      <v-card-text class="pa-4">
        <!-- Filters -->
        <v-row class="mb-4">
          <v-col cols="12" md="2">
            <v-text-field v-model="filters.brought_by" label="Taken By" placeholder="Search Taken By"
        ...
        <v-card-text class="pa-6">
          <div id="printable-area">
            <div class="text-center mb-4">
              ...
              <h3 class="text-h6 font-weight-bold mt-2">Outgoing Bill Details</h3>
            </div>
            <v-table density="compact" class="border">
              <tbody>
                ...
                <tr>
                  <td class="font-weight-bold border">Taken By</td>
                ...
                <tr>
                  <td class="font-weight-bold text-center text-h6 border bg-grey-lighten-3" colspan="8">Outgoing Items</td>
                </tr>
                ...
                <tr>
                  ...
                  <td class="font-weight-bold border text-center">₹ {{ selectedRecord?.incoming_outgoing_payment[0]?.payment_amount }}</td>
                ...
                <tr>
                  ...
                  <td class="font-weight-bold border text-center" colspan="2" :class="paymentStatus.color">{{ paymentStatus.status }}</td>
                </tr>
              </tbody>
            </v-table>
            <div class="d-flex justify-space-between mt-8 pt-8">
              <div>Signature of buyer ({{ selectedRecord?.brought_by || '_________' }})</div>
              <div>Signature of seller</div>
            </div>
          </div>
        </v-card-text>
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-form @submit.prevent="updateForm">
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field v-model="editForm.io_date" label="IO Date" type="date" variant="outlined" density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field v-model="editForm.brought_by" label="Taken By" variant="outlined" density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field v-model="editForm.mob_no" label="Mobile No" variant="outlined" density="compact"
                  maxlength="10" :error-messages="mobileError ? 'Invalid mobile number' : ''"
                  @input="validateMobile" required></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="d-flex ga-2">
                  <v-text-field v-model="editForm.vehicle_no_prefix" label="Vehicle Prefix" placeholder="CG04QB"
                    ...
                    :error-messages="vehicleNoPrefixError ? 'Invalid prefix' : ''"
                    @input="sanitizeVehicleNoPrefix" required></v-text-field>
                  <v-text-field v-model="editForm.vehicle_no_suffix" label="Suffix" placeholder="1234"
                    ...
                    :error-messages="vehicleNoSuffixError ? 'Invalid suffix' : ''"
                    @input="sanitizeVehicleNoSuffix" required></v-text-field>
                </div>
              </v-col>
              <v-col cols="12" md="6">
              ...
              <v-col cols="12" md="6">
                <v-text-field v-model="editForm.party_through" label="Party Through" variant="outlined" density="compact"
                  @input="editForm.party_through = $event.target.value.toUpperCase()"></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
            ...
            <v-table density="compact" class="border">
              ...
              <tbody>
                <tr v-for="(item, index) in editForm.incoming_outgoing_items" :key="index"
                  ...
                  <td>{{ item.amount || '-' }}</td>
                  <td>
                    <v-icon color="primary" size="small" @click="editItem(index)" class="mr-2 cursor-pointer">mdi-pencil</v-icon>
                    <v-icon color="error" size="small" @click="deleteItem(index)" class="cursor-pointer">mdi-delete</v-icon>
                  </td>
                </tr>
        ...
        <v-card-text class="pa-4">
          <v-form @submit.prevent="saveItem">
            <v-row>
              ...
              <v-col cols="12" md="6">
                <v-select v-model="modalData.bags_no" :items="[{ title: 'Bags', value: true }, { title: 'Nos', value: false }]"
                  label="Bags / Nos" item-title="title" item-value="value" variant="outlined" density="compact"
                  required></v-select>
              </v-col>
              <v-col cols="12" md="6">
...
<script>
import { outgoingService, firmDetailsService } from '@/services'
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { title: 'Taken By', key: 'brought_by', align: 'center' },
  ...
  methods: {
    ...
    async fetchRecords() {
      ...
      this.loading = true
      try {
        const response = await outgoingService.getOutgoings();
        if (Array.isArray(response.data)) {
          this.records = response.data.filter((record) => record.is_incoming === false)
        } else {
          this.records = []
    ...
    async openEditModal(record) {
      ...
      this.errorMessage = ''
      try {
        const response = await outgoingService.getOutgoingById(record.id)
        ...
        this.editForm = {
          ...
          is_incoming: data.is_incoming ?? false,
    ...
    async updateForm() {
      ...
      try {
        ...
        const response = await outgoingService.updateOutgoing(this.editForm.id, payload)
