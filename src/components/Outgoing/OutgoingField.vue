          ...
          <v-card-text class="pa-4">
            <v-form @submit.prevent="submitForm" ref="formRef">
              <v-row>
                ...
                <!-- Taken By -->
                <v-col cols="12" md="6">
                  <v-text-field v-model="form.brought_by" label="Taken By" placeholder="Enter Taken by"
                    variant="outlined" density="compact"
  ...
  methods: {
    ...
    async fetchModuleStatus() {
      try {
        ...
        const outgoingModule = modules.find((m) => m.module_name === 'outgoing')
        if (!outgoingModule) {
          this.isModuleEnabled = false
          this.toast.error('Outgoing module not found. Contact support.')
          return
        }
        this.isModuleEnabled = outgoingModule.module_enabled
      } catch (error) {
    ...
    async submitForm() {
      if (!this.isModuleEnabled) {
        this.toast.info('Please purchase the Outgoing module to add a outgoing.')
        return
      }
      if (!this.isFormValid) {
